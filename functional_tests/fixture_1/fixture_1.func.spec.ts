...
describe('Fixture 1: just bundling the assets', function () {
  ...
  before('Get archive entries', function () {
    const zip = new AdmZip(resolve(__dirname, 'dist', 'fixture_1.war'));
  ...
  describe('Webpack output', function () {
    ...
    it('Should output the correct archive information line', function () {
      expect(webpackOutput).to.contain('fixture_1.war');
      expect(webpackOutput).to.contain('[written]');
      expect(webpackOutput).to.match(/[\s\S]fixture_1\.war\t\d+\.?\d* (B|KB|MB)[\s\S]/);
    });
  });
