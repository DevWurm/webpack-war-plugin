...
describe('Fixture 4: adding additional elements and a WEB-INF folder', function () {
  ...
  before('Get archive entries', function () {
    const zip = new AdmZip(resolve(__dirname, 'dist', 'fixture_4.war'));
  ...
  describe('Webpack output', function () {
    ...
    it('Should output the correct archive information line', function () {
      expect(webpackOutput).to.contain('fixture_4.war');
      expect(webpackOutput).to.contain('[written]');
      expect(webpackOutput).to.match(/[\s\S]fixture_4\.war\t\d+\.?\d* (B|KB|MB)[\s\S]/);
    });
  });
